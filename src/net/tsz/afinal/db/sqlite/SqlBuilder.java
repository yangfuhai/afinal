...
public class SqlBuilder {
	...
	public static List<KeyValue> getSaveKeyValueListByEntity(Object entity) {
		...
		if(table.getId().getIdType() != IdType.AUTOINCREMENT) {// 用了非自增长,添加id , 采用自增长就不需要添加id了
			KeyValue kv = null;
			if (table.getId().getIdType() == IdType.UUID) {
				// UUID
				kv = new KeyValue(table.getId().getColumn(), UUID
						.randomUUID().toString());
				
			}
			else if(table.getId().getIdType() == IdType.NOTHING) {
				// 没有特殊处理
				kv = new KeyValue(table.getId().getColumn(), idvalue);
			}
			if(kv != null) {
				keyValueList.add(kv);
			}
		...
		for (Property property : propertys) {
			...
			if (kv != null) {
				keyValueList.add(kv);
				Log.d("debug", property.getColumn()+" "+property.getValue(entity));
			}
		}
...
//			strSQL.append("\"").append(id.getColumn()).append("\"    ").append("TEXT PRIMARY KEY,");
		...
		if (id.getIdType() == IdType.AUTOINCREMENT) {
			// 自增键
			strSQL.append("\"").append(id.getColumn()).append("\"    ")
					.append("INTEGER PRIMARY KEY AUTOINCREMENT,");
		} else if (id.getIdType() == IdType.UUID) {
			// uuid键
			strSQL.append("\"").append(id.getColumn()).append("\"    ")
					.append("VARCHAR(40) PRIMARY KEY,");
		} else if (id.getIdType() == IdType.NOTHING) {
			// 自定义键
			strSQL.append("\"").append(id.getColumn()).append("\"    ")
